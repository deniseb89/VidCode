...
var uploadAudio = function (ev) {
    var files = ev.target.files;
    var maxSize = 10000000;
    var fileTypes = ['mp3','ogg','wav'];
    for (var i = 0; i < files.length; i++) {
        var file = files[i];
        var ext = file.name.split('.').pop();
        console.log(ext);
...
var drawVideoFrame = function (time) {
    ...
    captureTxt = Math.floor( (100 * frames.length) / (vidLen *24)) + '%';
    $('.dl-progress').css('width', captureTxt);
    var frame = canvas.toDataURL('image/webp', 1);
    if (frame != frames[Math.max(0,frames.length-1)]) {
        frames.push(frame);
    }
};
...
var stopDL = function () {
    ...
    movie.play();
    movie.loop = true;
    webmBlob = Whammy.fromImageArray(frames, frames.length/vidLen);        
    console.log('num of frames generated by Whammy: '+frames.length);
    console.log('video is '+vidLen+' seconds long.');
    console.log('video saved at speed: '+frames.length/vidLen);
